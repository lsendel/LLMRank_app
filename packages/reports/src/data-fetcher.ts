...
import type { RawDbResults } from "./data-aggregator";
    ...
    historyCrawls.map(async (hc) => {
      const scores = await db
        .select({
          ...
          avgPerf: sql<number>`avg(${pageScores.lighthousePerf})`,
          avgSeo: sql<number>`avg(${pageScores.lighthouseSeo})`,
        })
        .from(pageScores)
        .where(eq(pageScores.jobId, hc.id));

      const s = scores[0];
      const perf = Number(s?.avgPerf ?? 0);
      const seo = Number(s?.avgSeo ?? 0);
      return {
        id: hc.id,
        ...
        avgPerformance: Math.round(((perf + seo) / 2) * 100),
      };
    }),
  );
