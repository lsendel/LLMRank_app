...
impl Config {
    pub fn from_env() -> Result<Self, ConfigError> {
        let shared_secret = env::var("SHARED_SECRET")
            .map_err(|_| ConfigError::Missing("SHARED_SECRET"))?;
        ...
        let r2_endpoint = env::var("R2_ENDPOINT")
            .map_err(|_| ConfigError::Missing("R2_ENDPOINT"))?;
        let r2_bucket = env::var("R2_BUCKET")
            .map_err(|_| ConfigError::Missing("R2_BUCKET"))?;
        ...
        let max_concurrent_fetches = env::var("MAX_CONCURRENT_FETCHES")
            ...
            .parse::<usize>()
            .map_err(|_| ConfigError::InvalidValue("MAX_CONCURRENT_FETCHES", "must be a valid usize"))?;

        let max_concurrent_lighthouse = env::var("MAX_CONCURRENT_LIGHTHOUSE")
            .unwrap_or_else(|_| "2".to_string())
            .parse::<usize>()
            .map_err(|_| ConfigError::InvalidValue("MAX_CONCURRENT_LIGHTHOUSE", "must be a valid usize"))?;
